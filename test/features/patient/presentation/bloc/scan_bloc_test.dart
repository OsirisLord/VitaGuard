...
void main() {
  ...
  blocTest<ScanBloc, ScanState>(
    ...
    build: () {
      ...
      when(() => mockRepository.analyzeXray(
              image: any(named: 'image'), patientId: any(named: 'patientId')))
  ...
  blocTest<ScanBloc, ScanState>(
    ...
    build: () {
      when(() =>
          mockRepository.analyzeXray(
              image: any(named: 'image'),
              patientId: any(named: 'patientId'))).thenAnswer(
          (_) async => const Left(ServerFailure(message: 'Analysis failed')));
      return bloc;
    },
    act: (bloc) => bloc.add(ScanImagePicked(tImage)),
    expect: () => [
